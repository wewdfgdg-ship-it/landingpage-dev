# MCP 레지스트리 — Objection Killer Agent
...
- **규칙 매핑 검증**: resistanceMap → ObjectionType 매핑의 논리적 일관성 분석
- **injectAt 매핑 검증**: structure[]와의 매핑 정확성 분석
- **거의 사용하지 않음** — 규칙 엔진의 로직은 단순한 편

### 사고 깊이
| 플래그 | 토큰 | 용도 |
|--------|------|------|
| `--think` | ~4K | 단일 매핑 규칙 디버깅 |
| `--think-hard` | ~10K | 전체 규칙 테이블 검증 |
...
### 이 에이전트에서의 사용
- **사용하지 않음** — Objection Killer 엔진은 순수 규칙 엔진, UI 없음
