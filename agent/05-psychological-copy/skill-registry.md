# 스킬 레지스트리 — Psychological Copy Agent

## 이 에이전트의 스킬 우선순위

| 순위 | 스킬 | 주요 용도 | 사용 빈도 |
|------|------|----------|----------|
| 1 | **/implement** | 엔진 코드 구현 (index.ts, prompts.ts, types.ts, frames.ts, tone-matrix.ts, quality-gate.ts) | 높음 |
| 2 | **/improve** | 카피 품질 개선, 프롬프트 최적화, 품질 게이트 조정 | 높음 |
| 3 | **/analyze** | 카피 품질 분석, 프레임 적합도 평가, 비용 분석 | 중간 |
| 4 | **/test** | 엔진 단위 테스트, 품질 게이트 검증, 재시도 로직 테스트 | 중간 |
| 5 | **/troubleshoot** | AI 호출 실패 디버깅, JSON 파싱 에러 진단, 품질 게이트 FAIL 원인 | 낮음 |
...
- **MCP**: Context7 (Claude SDK), Sequential (복잡 로직)
...
### Psychological Copy 엔진 구현
```
/design (프롬프트 구조 + 프레임/톤 설계)
    → /implement (frames.ts + tone-matrix.ts + quality-gate.ts + prompts.ts + index.ts + types.ts)
    → /analyze (카피 품질 + 비용 분석)
    → /test (품질 게이트 테스트 + 재시도 테스트)
    → /build (빌드 검증)
```

### 카피 품질 개선
```
/analyze (품질 게이트 FAIL 원인 분석)
    → /improve (프롬프트 수정 + 프레임 매핑 개선)
    → /test (재시도 로직 검증)
    → /build (빌드 확인)
```
