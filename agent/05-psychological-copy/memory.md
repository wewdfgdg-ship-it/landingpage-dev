...
### 프로젝트 특이사항
- 이 엔진은 Phase 2 첫 번째 AI 엔진으로, 실패 시 ⑥, Bridge, Image Gen, ⑩⑪ 블록됨
- Claude Sonnet ×1 호출 + 품질 게이트 (≥80점) + 재시도 (max 2)
- 한국어 시스템 프롬프트 + JSON prefill 필수
- 7가지 설득 프레임 (frames.ts) + 9개 업종별 톤 (tone-matrix.ts) 조합
- qualityScore = avgFrameScore × 0.6 + avgToneScore × 0.4
- headline ≤ 15자 엄수
- 재시도 시 실패 섹션만 재생성 (mergeCopy)
- 비용 합산 정확도 중요 (1차 + 재시도 호출 개별 추적)
