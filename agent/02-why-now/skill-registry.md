...
| 2 | **/test** | 순수 함수 단위 테스트, 규칙 매핑 검증 | 중간 |
| 3 | **/analyze** | 규칙 매핑 커버리지 분석, 타입 호환 검증 | 중간 |
| 4 | **/troubleshoot** | 규칙 매핑 실패 디버깅, 타입 불일치 진단 | 낮음 |
| 5 | **/improve** | 규칙 테이블 최적화, 코드 품질 개선 | 낮음 |
...
|-----------------------|-------------------|
| 엔진 구현 | /implement → /test → /build |
| 규칙 디버깅 | /troubleshoot → /implement → /test → /build |
...
| 3단계 (index.ts) | /implement → /test | --seq (선택) |
| 4단계 (pipeline.ts) | /implement | — |
| 5단계 (검증+검수) | /test → /analyze | --seq (FAIL 시) |

### → mcp-registry.md 자동 활성화 참조

| 스킬 실행 시 | MCP 자동 활성화 조건 |
|------------|-------------------|
| /troubleshoot | Sequential (복잡한 규칙 분석) |
