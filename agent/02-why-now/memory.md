...
- primaryType은 5가지 유효값: season, trend, scarcity, price, social_proof
- ctaUrgencyLevel 1~5 범위 클램핑 필수
- urgencyElements 최소 1개 보장 (빈 배열 금지)
- ③ Conversion Strategy가 후속 엔진이나, 실패해도 ③은 진행 가능 (비차단)
- 기본값 fallback으로 절대 throw하지 않는 안전한 엔진
...
체크 1: _shared/tool-broadcast.md 읽기
    └── target_agents에 "2" 포함된 [NEW_TOOL] 블록 있음?
