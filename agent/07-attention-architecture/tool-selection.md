# 도구 선택 — Attention Architecture Agent
...
### 주요 작업 시퀀스: Attention Architecture 엔진 구현

```
1. Read(src/engine/07-attention-architecture/types.ts)
   → 입출력 타입 구조 확인
   │
2. Write(src/engine/07-attention-architecture/rules.ts)
   → hookType/gazePattern/Zone 결정 규칙
   │
3. Write(src/engine/07-attention-architecture/index.ts)
   → runAttentionArchitecture 함수 구현
   → 4 Zone 생성 + ratio 정규화 + pixelRange 순차 계산
   │
4. Bash("npx tsc --noEmit") → 타입 체크
   │
5. Bash("npm run build") → 빌드 검증
```
...
| Zone 검증 | Read → Edit → Bash(tsc) | ratio/pixelRange 수학 검증 |
