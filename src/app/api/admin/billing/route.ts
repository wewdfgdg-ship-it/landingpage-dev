...
async function getSubscriptions({ status, plan, skip, limit }: ListParams): Promise<{
  items: { id: string; orgId: string; orgName: string; plan: string; status: string; currentPeriodStart: Date | null; currentPeriodEnd: Date | null; cancelAtPeriodEnd: boolean; cancelReason: string | null; createdAt: Date }[];
  total: number;
  page: number;
  totalPages: number;
}> {
...
async function getPayments({ status, skip, limit }: Omit<ListParams, 'plan'>): Promise<{
  items: { id: string; orgId: string; orgName: string; amount: number; status: string; provider: string; externalId: string | null; metadata: unknown; createdAt: Date }[];
  total: number;
  page: number;
  totalPages: number;
}> {
